 ...
 * uniform, symmetric link costs in a ring topology.
 */
class testcase_sp_uniform_ring final : public testcase {
private:
    std::vector<uint64_t> received_;
    std::vector<mixnet_address> expected_0_{42, 21};
    std::vector<mixnet_address> expected_1_{65534, 0};
    std::vector<std::string> data_{"Never gonna give you up",
                                   "Never gonna let you down"};
public:
    explicit testcase_sp_uniform_ring() :
        testcase("testcase_sp_uniform_ring"), received_(2, 0) {}
...
int main(int argc, char **argv) {
    testcase_sp_uniform_ring tc; // Run testcase
